...
void cuda_preprocess_gpu(uint8_t *src, int src_width, int src_height, size_t src_step,
                         ...
                         cudaStream_t stream)
{
    ...
    // gpu
    if (src_step == (size_t)(src_width * 3))
    {
        CUDA_CHECK(cudaMemcpyAsync(img_buffer_device, src, img_size, cudaMemcpyDeviceToDevice, stream));
    ...
    else
    {
        CUDA_CHECK(cudaMemcpy2DAsync(
            ...
            src_width * 3,            // dst pitch（连续，无padding）
            src,                      // src（GpuMat 设备指针）
            src_step,                 // src pitch（GpuMat step，可能有padding）
            src_width * 3,            // 每行有效数据宽度（bytes）
            src_height,               // 行数
            cudaMemcpyDeviceToDevice, // ⬅️ D2D
            stream));
    }
    AffineMatrix s2d, d2s;
    float scale = std::min(dst_height / (float)src_height, dst_width / (float)src_width);
...
void cuda_batch_preprocess_gpu(std::vector<cv::cuda::GpuMat> &img_batch,
                               float *dst, int dst_width, int dst_height,
                               cudaStream_t stream)
{
    ...
    for (size_t i = 0; i < img_batch.size(); i++)
    {
        cuda_preprocess_gpu(img_batch[i].ptr<uint8_t>(), img_batch[i].cols, img_batch[i].rows, img_batch[i].step, &dst[dst_size * i], dst_width,
                            dst_height, stream);
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
}
