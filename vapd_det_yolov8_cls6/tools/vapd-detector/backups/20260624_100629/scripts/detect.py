...
def main():
    ...
    parser.add_argument("--env", default="sit", choices=list(ENVS), help="环境（默认 sit）")
    parser.add_argument(
        "--upload",
        default="oss",
    ...
    if not video_url:
        if args.demo:
            ...
            print(f"[info] 未提供 URL，使用内置示例视频: {video_url[:80]}...", file=sys.stderr)
        else:
            parser.error("必须提供 video_url，或使用 --demo")
